...
class TransmissionRPC
{
  ...
  {
    if ( !is_array( $ids ) ) $ids = array( $ids );	// Convert $ids to an array if only a single id was passed
    if ( count( $fields ) == 0 ) $fields = array( "id", "name", "status", "doneDate", "haveValid", "totalSize", "percentDone", "hashString", "rateUpload", "rateDownload", "uploadRatio", "peersConnected" );	// Defaults
    $request = array(
      "fields" => $fields,
      "ids" => $ids
    );
